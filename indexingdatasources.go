...
func newIndexingDatasources(rootSDK *Glean, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *IndexingDatasources {
	return &IndexingDatasources{
		rootSDK:          rootSDK,
...
func (s *IndexingDatasources) Submit(ctx context.Context, datasourceInstance string, type_ string, requestBody map[string]any, opts ...operations.Option) (*operations.PostRestAPIIndexSubmissionsDatasourceInstanceTypeResponse, error) {
	request := operations.PostRestAPIIndexSubmissionsDatasourceInstanceTypeRequest{
	...
	baseURL := utils.ReplaceParameters(operations.PostRestAPIIndexSubmissionsDatasourceInstanceTypeServerList[0], map[string]string{
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/rest/api/index/submissions/{datasourceInstance}/{type}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.PostRestAPIIndexSubmissionsDatasourceInstanceTypeResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 202:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out operations.PostRestAPIIndexSubmissionsDatasourceInstanceTypeResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
