...
```go
apiclientgo.WithServerURL("https://api.example.com")
```
...
```go
apiclientgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
apiclientgo.WithClient(httpClient)
```
...
```go
apiclientgo.WithSecuritySource(/* ... */)
```
...
```go
apiclientgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
apiclientgo.WithTimeout(30 * time.Second)
```
