...
func newEntities(rootSDK *Glean, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Entities {
	return &Entities{
		rootSDK:          rootSDK,
...
func (s *Entities) RetrievePersonPhoto(ctx context.Context, personID string, ds *string, opts ...operations.Option) (*operations.GetPersonPhotoResponse, error) {
	request := operations.GetPersonPhotoRequest{
		PersonID: personID,
		Ds:       ds,
	}
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/rest/api/v1/people/{person_id}/photo", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.GetPersonPhotoResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
